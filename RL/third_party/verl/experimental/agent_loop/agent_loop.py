    ...
    async def generate(
        ...
        *,
        ...
        trace_lora_path(
            ...
            f"sampling_lora_int_id={sampling_params.get('_lora_int_id')} "
            "forwarded_lora_request=via_sampling_params"
        )
...
class AgentLoopWorker:
    ...
    @staticmethod
    def _normalize_prompt_ids(prompt_ids: Any) -> list[int]:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        if "temperature" in batch.meta_info:
            sampling_params["temperature"] = float(batch.meta_info["temperature"])
        if "response_length" in batch.meta_info:
            response_length = int(batch.meta_info["response_length"])
            if response_length <= 0:
                raise ValueError(f"Invalid rollout response_length={response_length}")
            sampling_params["max_tokens"] = response_length

        lora_int_id = batch.meta_info.get("_lora_int_id", None)
        if lora_int_id is not None:
            sampling_params["_lora_int_id"] = int(lora_int_id)

        trace_lora_path(
            "[AgentLoopWorker.generate_sequences] "
            f"batch_size={len(batch)} validate={batch.meta_info.get('validate', False)} "
            f"meta_lora_int_id={batch.meta_info.get('_lora_int_id')} "
            f"meta_keys={sorted(batch.meta_info.keys())} "
            f"sampling_keys_before_dispatch={sorted(sampling_params.keys())}"
        )
        ...
        for sample_idx, (agent_name, messages, trajectory) in enumerate(
            zip(agent_names, raw_prompts, trajectory_info, strict=True)
        ):
            prompt_ids = None
            if raw_prompt_ids is not None:
                prompt_ids = self._normalize_prompt_ids(raw_prompt_ids[sample_idx])
            trace_lora_path(
                "[AgentLoopWorker.generate_sequences.dispatch] "
                f"agent_name={agent_name} sample_index={trajectory.get('sample_index')} "
                f"batch_meta_lora_int_id={batch.meta_info.get('_lora_int_id')} "
                f"lora_arg_to_agent_loop={sampling_params.get('_lora_int_id')}"
            )
            tasks.append(
                asyncio.create_task(
    ...
    async def _run_agent_loop(
        ...
        with rollout_trace_attr(
            ...
            if prompt_ids is not None and agent_name == "single_turn_agent":
                ...
                metrics["generate_sequences"] = time.perf_counter() - start_time
                metrics["tool_calls"] = 0.0
                response_length = int(
                    sampling_params.get("max_tokens", self.config.actor_rollout_ref.rollout.response_length)
                )
            ...
            trace_lora_path(
                ...
                f"sampling_keys={sorted(sampling_params.keys())} "
                f"sampling_lora_int_id={sampling_params.get('_lora_int_id')} "
                "run_signature_lora_arg=via_sampling_params"
            )
