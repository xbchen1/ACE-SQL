...
# Monkey-patch vLLM LRUCache.touch() which references a renamed internal method.
# LRUCache.__update was renamed to __touch but touch() still calls __update.
try:
    from vllm.utils import LRUCache as _LRUCache
    if hasattr(_LRUCache, '_LRUCache__touch') and not hasattr(_LRUCache, '_LRUCache__update'):
        _LRUCache._LRUCache__update = _LRUCache._LRUCache__touch
except Exception:
    pass
...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def init_engine(self):
        ...
        is_lora = lora_rank > 0
        is_dual_lora = is_lora and config.model.get("dual_lora", {}).get("enabled", False)
        self.is_lora = bool(is_lora)
        self.is_dual_lora = bool(is_dual_lora)
        if is_dual_lora:
            dual_cfg = config.model.get("dual_lora", {})
            lora_rank = max(
                int(lora_rank),
                int(dual_cfg.get("ret_lora_rank") or lora_rank),
                int(dual_cfg.get("gen_lora_rank") or lora_rank),
            )
        ...
        lora_engine_kwargs = {}
        if is_lora:
            max_loras = 2 if is_dual_lora else 1
            lora_engine_kwargs = {
                "enable_lora": True,
                "max_loras": max_loras,
                "max_lora_rank": lora_rank,
                "lora_dtype": _vllm_lora_dtype_name(config.get("dtype", "auto")),
            }
            print(f"AsyncvLLMServer: LoRA enabled, max_loras={max_loras}, max_lora_rank={lora_rank}")
        _trace_lora_path(
            "[AsyncvLLMServer.init_engine] "
            f"is_lora={is_lora} is_dual_lora={is_dual_lora} "
            f"lora_rank={lora_rank} lora_engine_kwargs={lora_engine_kwargs}"
        )
    ...
    async def generate(self, prompt_ids: list[int], sampling_params: dict[str, Any], request_id: str) -> list[int]:
        ...
        if remaining_tokens < 1:
            raise ValueError(
                f"Prompt length {len(prompt_ids)} must be smaller than max_model_len={self.max_model_len}."
            )
        sampling_params = dict(sampling_params)
        requested_max_tokens = sampling_params.pop("max_tokens", None)
        if requested_max_tokens is None:
            requested_max_tokens = sampling_params.pop("max_new_tokens", None)
        if requested_max_tokens is None:
            requested_max_tokens = int(self.config.rollout.response_length)
        requested_max_tokens = int(requested_max_tokens)
        if requested_max_tokens <= 0:
            raise ValueError(f"max_tokens must be positive, got {requested_max_tokens}.")
        max_tokens = min(requested_max_tokens, int(self.config.rollout.response_length), remaining_tokens)
        lora_int_id = sampling_params.pop("_lora_int_id", None)
        lora_request = None
        if self.is_lora:
            if lora_int_id is None:
                if self.is_dual_lora:
                    raise RuntimeError(
                        "Dual LoRA async rollout requires sampling_params['_lora_int_id'] "
                        "to select retriever(1) or generator(2); got None."
                    )
                lora_int_id = 1
            lora_int_id = int(lora_int_id)
            if self.is_dual_lora and lora_int_id not in {1, 2}:
                raise RuntimeError(
                    f"Dual LoRA async rollout only accepts LoRA ids 1(retriever) and 2(generator); got {lora_int_id}."
                )
            if not hasattr(self.engine, "list_loras"):
                raise RuntimeError("LoRA async rollout cannot verify loaded adapters: engine has no list_loras().")
            loaded_loras = await self.engine.list_loras()
            loaded_lora_ids = sorted(int(lora_id) for lora_id in loaded_loras)
            if lora_int_id not in loaded_lora_ids:
                mode = "Dual-LoRA" if self.is_dual_lora else "Single-LoRA"
                raise RuntimeError(
                    f"Requested {mode} id {lora_int_id} is not loaded in async vLLM. "
                    f"Available LoRA ids: {loaded_lora_ids}"
                )
        ...
        elif lora_int_id is not None:
            raise RuntimeError(f"Received LoRA id {lora_int_id}, but rollout LoRA is disabled.")
        _trace_lora_path(
            "[AsyncvLLMServer.generate.before_engine] "
            f"request_id={request_id} prompt_len={len(prompt_ids)} "
            f"max_tokens={max_tokens} requested_max_tokens={requested_max_tokens} "
            f"sampling_keys={sorted(sampling_params.keys())} "
            f"sampling_lora_int_id={lora_int_id} "
            f"engine_generate_lora_request={lora_int_id if lora_request is not None else None}"
        )
